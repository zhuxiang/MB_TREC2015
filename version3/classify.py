...
class Classifier:
    ...
    def classify(self, tweet_text):
        ...
        if self.max_similarity[topic_id] < similarity and similarity > 1.0:
            self.max_similarity[topic_id] = similarity
            for word in tweet_keywords:
                if word in self.keyword_map:
                    self.keyword_map[topic_id][word] += similarity
                else:
                    self.keyword_map[topic_id][word] = similarity
            self.keyword_map[topic_id] = dict(sorted(self.keyword_map[topic_id].iteritems(), key=lambda item: item[1], reverse=True))
    ...
    def clf(self, tweet, profiles):
        ...
        for index, profile in profiles:
            profile.update(self.keyword_map[index].keys()[:2])
